...
```
Yêu cầu tổ chức không xử lý an ninh mạng theo kiểu bị động và rời rạc, mà phải quản trị rủi ro như một danh mục sống: luôn biết rủi ro nào đang tồn tại, mức độ ra sao, ai chịu trách nhiệm, xử lý bằng gì, còn tồn dư gì và khi nào phải báo cáo. Đặt trong bối cảnh này FDS chưa có nhận thức đầy đủ, chưa kiểm soát tốt vấn đề này.
```
...
```
Yêu cầu tổ chức kiểm soát được mọi thiết bị có mặt trong hệ thống: thiết bị nào tồn tại, ở đâu, thuộc ai, có hợp lệ không, có cần bảo vệ không, có kết nối trái phép không và khi rời khỏi hệ thống thì dữ liệu đã được xóa an toàn chưa. Đặt trong bối cảnh này FDS chưa có nhận thức đầy đủ, chưa kiểm soát tốt vấn đề này.
```
...
```
Yêu cầu tổ chức kiểm soát được toàn bộ phần mềm đang tồn tại trong hệ thống: cái gì được phép, cái gì đang cài, cái gì hết hỗ trợ, cái gì trái phép, ai chịu trách nhiệm và xử lý như thế nào. Đặt trong bối cảnh này FDS chưa có nhận thức đầy đủ, chưa kiểm soát tốt vấn đề này. `
```
...
```
Yêu cầu tổ chức phải quản trị dữ liệu như một tài sản có chủ sở hữu, có mức nhạy cảm, có quyền truy cập, có vòng đời, có mã hóa, có audit và có kiểm soát chống thất thoát. Đặt vào bối cảnh này, FDS chưa có nhận thức đầy đủ về vấn đề này
```
...
```
Đáp ứng việc quản lý session trên các sản phẩm phần mềm của FDS. Các vấn đề khác hiện tại chưa từng triển khai và chưa đánh giá.
```
...
```
Hiện tại FDS đã có quy trình SOP tương đương, việc triển khai ap dụng cũng đã bước đầu được thực hiện. Tuy nhiên tiến độ và kết quả chưa đáp ứng được yêu cầu của tiêu chuẩn. 
```
...
```
Bản chất ở đây là một quy trình liên tục nhằm phát hiện, đánh giá, ưu tiên, khắc phục và xác minh các điểm yếu bảo mật trên toàn bộ tài sản công nghệ thông tin. 
Phát hiển bằng cách nào?
- Dựa vào danh mục tài sản và các phiên bản phần mền trên tài sản đó, qua đó sử dụng cơ sở dữ liệu CVE để đối chiếu
- Dựa vào công cụ để rà quét hạ tầng, thiết bị và phần mềm, mã nguồn
- Dựa vào việc kiểm soát chặt chẽ cấu hình hệ thống. Nhiều lỗ hổng xuất phát từ việc cấu hình sai hoặc để cấu hình mặc định của nhà sản xuất
- Dựa vào log, SIEM và dấu hiệu khai thác
- Dựa vào kiểm thử xâm nhập
Đánh giá thế nào?
- Sử dụng điểm CVSS
- Khả năng khai thác, việc đã có mã khai thác công khai hay chưa, mức độ phơi lộ của tài sản, độ quan trọng của tài sản, mức độ nhạy cảm của dữ liệu, tác động đến tính bí mật, toàn vẹn, sẵn sàng và các kiểm soát bù hiện có như firewall, WAF, segmentation, EDR hoặc cơ chế giám sát.
Khắc phục thế nào?
- Áp dụng bản vá
- Thay đổi cấu hình
- Cô lập hoặc tắt tài sản
- Thay thế tài sản

Kiểm tra lại thế nào?
- Thực hiện kiểm thử lại để xác minh lỗ hổng đã được khắc phục hoàn toàn.
- Cập nhật lại danh mục tài sản với thông tin phiên bản mới.
- Theo dõi log và các cảnh báo bảo mật liên quan để đảm bảo không có dấu hiệu khai thác.
Tóm lại việc quản lý lỗ hổng bảo mật không hề đơn giản, nó đòi hỏi sự phối hợp nhịp nhàng giữa các bộ phận, sự đầu tư về con người và công nghệ, và quan trọng nhất là sự cam kết của lãnh đạo. Đặt trong bối cảnh này FDS gần như chưa đáp ứng được tiêu chí nào.
```
...
```
Về mặt kỹ thuật, cần triển khai nền tảng thu thập và lưu trữ log tập trung như SIEM hoặc hệ thống log management, kết nối log từ firewall, WAF, VPN, máy chủ, ứng dụng, cơ sở dữ liệu, IAM, EDR và các dịch vụ của nhà cung cấp. Đặt vào bối cảnh này FDS chưa có kinh nghiệm thiết lập và vận hành.
```
...
```
FDS không có kinh nghiệm, kiến thức vận hành và bảo vệ các hệ thống mail
```
...
```
Cần phải có năng lực phòng ngừa, phát hiện, ngăn chặn, cô lập, điều tra và khắc phục mã độc trên máy trạm, máy chủ, thiết bị lưu trữ ngoài, điểm kết nối nội bộ và điểm kết nối ra bên ngoài. Nếu đặt vào bối cảnh này thì FDS xây dựng quy trình và có nhân sự chuyên trách.
```
...
```
FDS đáp ứng về khẳ năng thiết kế và thực thi ở những hệ thống vừa và nhỏ, các hệ thống dữ liệu lớn hàng trăm GB trở lên sẽ gặp khó khăn nếu không có các license thương mại, điều này không phải do năng lực mà do hạn chế của opensource.
```
...
```
Đối với các hệ thống nhỏ, đơn lẻ thì FDS có khả năng thiết kế, vận hành. Tuy nhiên các hệ thống lớn thì cần nâng cao năng lực về thiết kế network, am hiểu tường tận các loại thiết bị phần cứng, có khả năng kiến trúc và xây dựng quy trình vận hành
```
...
```
Để đáp ứng các yêu cầu này cần: Con người → Quy trình → Kiến trúc giám sát → Nguồn dữ liệu → Công cụ phòng thủ → Kịch bản cảnh báo/vận hành 24/7
Trong đó:
- Con người: có chuyên môn, nghiệp vụ, kinh nghiệm vận hành, xử lý các sự cố an ninh mạng.
- Quy trình: có quy trình vận hành, xử lý các sự cố an ninh mạng.
- Kiến trúc giám sát: có hệ thống giám sát an ninh mạng, an toàn hệ thống thông tin(PAM, SIEM, SOC...), có khả năng thiết lập và tùy chỉnh
- Nguồn dữ liệu: có nguồn dữ liệu để rà soát và đưa ra các cảnh báo.(IDS/IPS, FW, các hệ thống thông tin, CSDL, ứng dụng...)
- Công cụ phòng thủ: có công cụ phòng thủ để ngăn chặn xâm nhập(IPS, NAC, WAF, Sandbox, Firewall, Antivirus...), có khả năng sử dụng, khai thác tối đa khả năng của công cụ.
- Kịch bản cảnh báo/vận hành 24/7: có kịch bản cảnh báo/vận hành 24/7 và được diễn tập định kỳ. Được cập nhật, cải tiến định kỳ.
→ FDS chưa đáp ứng điều kiện nào nếu đặt trong bối cảnh phải vận hành một hệ thống hiện đại với yêu cầu cao về an ninh mạng.
```
...
```
Đối với FDS chưa có khung năng lực chuyên trách, chưa có nhân sự chuyên trách, hiện tại chỉ có vai trò systemadmin kiêm nhiệm.
```
...
```
Dành cho các đơn vị chủ quản hệ thống thông tin.
```
...
```
Bên phát triển: Xây dựng phần mềm an toàn
- Kiến trúc ứng dụng tính đế bảo mật(Đã có tính đến trong giai đoạn thiết kế, tuy nhiên vẫn cần nâng cấp để  phù hợp hơn với gia đoạn hiện nay)
- Lập trình theo nguyên tắc secure coding(Hiện tại đang rà quét mã nguồn bằng sonarQube, gitleak, cần bổ sung thêm các công cụ khác, quy trình cụ thể)
- Quản lý thư viện và mã nguồn bên thứ ba(Chưa đáp ứng)
- Có kiểm thử bảo mật tự động và thủ công(Chưa đáp ứng)
- Tách biệt môi trường phát triển, kiểm thử và vận hàn(Đã đáp ứng)
- Có quy trình xử lý lỗ hổng sau khi phát hiện(Đã đáp ứng)
Bên vận hành:Quản lý pipeline CI/CD; kiểm soát quyền deploy; lưu secret trong vault; ký và kiểm soát artifact; tách môi trường; chỉ cho phép image/package đã scan được triển khai; kiểm soát rollback và release approval.
```
...
```
Bên phát triển: Thiết kế ứng dụng theo hướng có thể quan sát được, có thể điều tra được và có thể hỗ trợ xử lý sự cố nhanh chóng
    - Thiết kế khả năng truy vết giao dịch end-to-end(Đã đáp ứng trên ht mới đã có như: CSDLDC)
    - Chuẩn hóa log ứng dụng(Theo 11930 và 742, đã đáp ứng)
    - Ghi nhận audit log cho hành vi quan trọng(Đã đáp ứng trên ht mới đã có như: CSDLDC)
    - Thiết kế cảnh báo cấp ứng dụng(Chưa đáp ứng)
    - Hỗ trợ điều tra và khắc phục sự cố(Các hệ thống mới đều lưu log tập trung, sẵn sàng việc truy vết)
    
Bên vận hành hệ thống: Tổ chức năng lực ứng phó sự cố theo hướng có người phụ trách, có quy trình, có kênh liên lạc, có công cụ giám sát, có ngưỡng cảnh báo, có diễn tập và có đánh giá sau sự cố.(Đặt ngữ cảnh vào FDS thì cần chuẩn hóa quy trình và bổ sung nhân lực chuyên trách)
...
```
Bên phát triển phần mềm: Hỗ trợ kiểm thử API, web, mobile, source code; cung cấp tài liệu kiến trúc, tài khoản test, API spec; xử lý phát hiện từ pentest; cập nhật backlog bảo mật; kiểm tra lại sau khi vá.

Bên vận hành: Lập kế hoạch pentest định kỳ tối thiểu hằng năm; xác định phạm vi, giới hạn, thời gian, cách phản ứng; thuê/bố trí đơn vị độc lập; kiểm thử từ Internet và nội bộ; rà soát biện pháp bảo mật sau kiểm thử; theo dõi đóng lỗi.
